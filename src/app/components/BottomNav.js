...
export default function ResponsiveNav() {
  ...
  const staffNavItems = [
    ...
    { href: '/attendance', label: 'Attendance', icon: <FiCalendar /> },
    { href: '/expenses', label: 'Expenses', icon: <FiFolder /> },
  ];

  useEffect(() => {
    const role = localStorage.getItem('userRole');
    setNavItems(role === 'staff' || role === 'staff123' ? staffNavItems : ownerNavItems);
  }, []);
  ...
  return (
    <>
      ...
      <nav className="hidden md:flex justify-between items-center bg-white px-6 py-4 shadow fixed top-0 left-0 right-0 z-50">
        <div className="text-xl font-bold text-blue-600">🍽️ MyRestaurant</div>
              ...
              >
                <button
                  className={`flex items-center gap-1 ${
                    pathname.startsWith('/staff') ? 'text-blue-600' : 'text-gray-600'
                  }`}
                >
                  {item.icon}
          ...
          item.children ? (
            <button
              ...
              className={`flex flex-col items-center ${
                pathname.startsWith('/staff') ? 'text-blue-600' : 'text-gray-500'
              }`}
            >
      ...
      <AnimatePresence>
        {showMobileSheet && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 bg-cyan-500 border-t shadow-lg z-50 rounded-t-lg"
          >
            <div className="px-4 py-4 border-b font-bold">Staff Menu</div>
            {navItems.find((i) => i.children)?.children.map((child) => (
              <Link
                key={child.href}
                href={child.href}
