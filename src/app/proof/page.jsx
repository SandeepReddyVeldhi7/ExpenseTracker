...
export default function ProofHome() {
  ...
  useEffect(() => {
    fetch("/api/v1/proof/get-submitted-dates")
      .then((r) => r.json())
      .then((d) => setSubmittedDates(d.dates.map((x) => new Date(x))));
  }, []);

  const handleNext = () => {
    if (selectedDate) {
      const formatted = selectedDate.toLocaleDateString("en-CA");
      router.push(`/proof/${formatted}`);
    }
  };
  ...
  return (
    <div className="min-h-[90vh] bg-gradient-to-br from-black via-gray-800 to-lime-800 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <h1 className="text-2xl font-bold mb-6 text-center text-gray-800">Select Proof Date</h1>
